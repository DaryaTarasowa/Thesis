\documentclass[PhD, Submit, ngerman,UKenglish,table]{scrbook}
...
\newacronym{ocw}{OCW}{OpenCourseWare}
...
\newacronym{mmq}{MMQ}{Multiple-Mark Questions}
\newacronym{mcq}{MCQ}{Multiple-Choice Questions}
\newacronym{mtf}{MTF}{Multiple True-False questions}
\newacronym{lms}{LMS}{Learning Management Systems}
\newacronym{rdf}{RDF}{Resource Description Framework}
\newacronym{rdb2rdf}{RDB2RDF}{Relational Data Base to \gls{rdf}}
\newacronym{r2rml}{R2RML}{Relational to \gls{rdf} Mapping Language}
\newacronym{sco}{SCO}{Sharable Content Objects}
\newacronym{cosmec}{CoSMEC}{Crowdsourcing (semantically) Structured Multilingual Educational Content}
\newacronym{cms}{CMS}{Content Management System}
\newacronym{lor}{LOR}{Learning Object Repository}
\newacronym{mvc}{MVC}{Model-View-Controller}
\newacronym{lamp}{LAMP}{Linux-Apache-MySQL-PHP}
\newacronym{w3c}{W3C}{World Wide Web Consortium}
\newacronym{sql}{SQL}{Search Query Language}
...
\newacronym{mern}{MERN}{MongoDB-ExpressJS-ReactJS-NodeJS}
...
\newcommand\RotText[1]{\rotatebox{90}{\parbox{3cm}{\raggedright#1}}}
...
\newcommand*{\thesisrefereetwo}{Prof.\ Dr.\ Oscar Corcho}
...
The support of multilingual content is crucial for educational platforms because of the many benefits it brings. %what benefits? Be specific
...
%Additionally, we were motivated by the fact, that although many of the approaches found in the literature were proven to be beneficial, the existing leading \gls{ocw} authoring platforms do not integrate them.
...
Localization & \\
\rowcolor{LightGray}
\multicolumn{2}{l}{Reuse and repurpose}\\
...
\toprule
\multirow{2}{*}{\backslashbox{\Rotatext{Technology}}{\Rotatext{Aspect}}} & \multicolumn{7}{e|}{Content co-creation} & 
\multicolumn{3}{y|}{Reuse and repurpose} &
...
\cmidrule(r){2-16}
   ...
   \RotText{Assessment items authoring} &
   \RotText{Quality assurance} &
   \RotText{Categorization} &
   \RotText{Personalization} &
   \RotText{Localization} &
   \RotText{Search etc.} &
   \RotText{Remixing} &
   \RotText{Organization} &
   \RotText{Negotiation} &
   \RotText{Awareness} &
...
%The list of benefits of using Semantic Wikis in \gls{ocw} production according to \citeauthor{mu2012research, Rodriguez-Artacho2011, Li2010a}:
%\begin{itemize}
...
%\item Collective participation of resources can solve the problems of the cost spent on the building and maintenance of \gls{oer};
%\item From the aspect of \gls{ocw} authoring these platforms allow to collaborate on the content and on course structure (or curriculum) at the same time.
...
%The predecessor of Semantic Wiki is a \gls{cms} build on ontology.
%An example of such system developed for authoring \gls{ocw} is \cite{dicheva2002collaborative}.
%
%
...
%\cite{Shih2008}
%\cite{hamid2010efficient} addresses the problem of low performance by implementing \gls{ocw} authoring platform using a Grid architecture.
...
%
%\toprule
%\textbf{Features} & IMS & AICC & \gls{scorm} & DublinCore \\
%\midrule
%
...
%\textbf{\gls{xml} Bindings} 			& \checkmark 	&  				& \checkmark  	& \checkmark \\
%\textbf{\gls{rdf} Bindings} 			& \checkmark 	& 				&  				& \checkmark \\
...
%\subsubsection{SCORM}
%\label{sec:SCORM}
%\cite{Guo2008} mentions that \gls{sco} can hardly impart complex facts adequately, that makes it unable to support adaptability of the produced \gls{ocw}.
%To overcome the limitation, authors propose to add a new extension to the specification of \gls{scorm} by utilizing state diagrams from UML 2.0 instead of \gls{scorm}-LSTD to describe the states and actions of training participants.
%The proposed \gls{lms} provides a learning process engine which behaves as a state machine while interpreting the sequencing and navigation rules.
...
%
%
%\subsubsection{SMIL} SMIL(Synchronized Multimedia Integration Language) is a \gls{xml} based markup language by \gls{w3c} recommendation to create multimedia presentations~\cite{bulterman2005synchronized}.
...
%
%\subsubsection{LRMI}
%Synergistic efforts of the Learning Resource Metadata Initiative (LRMI)~\footnote{\url{www.lrmi.net}}, funded by the Bill and Melinda Gates Foundation, establish a simplifi ed set of standards to tag all educational online content, making it easier to publish, discover, and deliver quality \gls{oer}s on the Web.
%
%\subsubsection{RDF}
%\gls{rdf} is an \gls{xml}-based language for describing information contained in a Web resource.
%This Web resource can be anything, for example a Web page or a Website.
%\gls{rdf} is the basic building block for supporting the Semantic Web, and is same as \gls{html} is for the conventional Web.
%
%The properties of \gls{rdf} are:
%\begin{itemize}
%\item \gls{rdf} is a language recommended by \gls{w3c}~\footnote{\url{http://www.w3.org/RDF/}} and it is all about metadata,
%\item \gls{rdf} is capable of describing any fact (resource) independent of any domain,
%\item \gls{rdf} provides a basis for \emph{coding}, \emph{exchanging}, and \emph{reusing} structured metadata,
%\item \gls{rdf} is structured; i.e. it is machine-understandable. Machines can do useful operations with the knowledge expressed in \gls{rdf},
%\item \gls{rdf} allows \emph{interoperability} among applications exchanging \emph{machine understandable} information on the Web.
%\end{itemize}
%
%Serializing \gls{rdf} data is a crucial issue since different platforms and environments work better with different data formats.
%There are already several formats such as \emph{\gls{rdf}/\gls{xml}}, \emph{Turtle}, \emph{N-Triples}, \emph{Notation3}, \emph{N-Quads} and \emph{\gls{json}-LD} for serializing \gls{rdf} data.
%There are also formats such as \emph{RDFa} and \emph{Microdata} to embed \gls{rdf} data within \gls{html} documents by offering additional \gls{html} attributes.
%
...
%An \gls{rss} document, which is called a ‘‘feed,” ‘‘Web feed,” or ‘‘channel,” contains either a summary of content from an associated Web site or the full text.
%\gls{rss} makes it possible for people to keep up with their favorite Web sites in an automated manner rather than checking them manually.
%\gls{rss} content can be read using a software tool called ‘‘\gls{rss} reader,” ‘‘feed reader” or ‘‘aggregator.”
%A user subscribes to a feed by entering the feed's link into the reader or by clicking an \gls{rss} icon in a browser that
...
%The proposed system translates each book into a \gls{rss} feed to help learners aggregate and read their learning materials
%in \gls{rss} readers.
%
%
...
\rowcolor{LightGray}
\multicolumn{3}{l}{Reuse and repurpose}\\
...
\textbf{18} & \textbf{14} \\
\end{tabular}
\caption{Incorporation of frequently used complicated queries results into the mapping.}
...
In order to increase the index, we have added machine-readable indication of a license, metadata about publisher and dataset itself.
...
For newly created properties dereferencable \gls{uri} were created.
...
%Since the initial \texttt{SlideWiki} design new approaches for enabling access to a web-site from various mobile devices have appeared.
